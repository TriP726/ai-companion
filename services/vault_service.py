...
class VaultService(BaseService):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        # True when the in-memory index has changes (currently: removals)
        # made while in PRIVATE mode that have not yet been written to
        # disk. Cleared once flushed on return to NORMAL mode. See
        # remove_file() and set_mode().
    ...
    def stop(self) -> None:
        # Never flush the index to disk while Private — see remove_file()
        # for why an entry can be dirty without being safe to persist yet.
        if self._vault_store and not self.is_private:
    ...
    def set_mode(self, mode: VaultMode, persist: bool = True) -> None:
        ...
        if old == VaultMode.PRIVATE and self._index_dirty and self._vault_store:
            # A removal happened while Private and was kept in-memory only.
            # Now that we're leaving Private mode, it's safe to write the
            # current (already-correct) index to disk rather than leaving
    ...
    def remove_file(self, vault_path: str) -> bool:
        ...
        The actual delete always happens immediately and unconditionally —
        a destructive operation the owner explicitly requested should never
        be silently deferred, in Private mode or otherwise.

        What Private mode changes is whether that removal is *written to
        disk*. Every other persistent service in the app suppresses disk
        writes while Private; this one didn't, and unconditionally called
        `_vault_store.save()` here regardless of mode. The in-memory index
        is still updated immediately either way, so `list_files()` and
        `get_file_path()` reflect the removal within the same session
        whether or not the write actually reached disk.
        """
        try:
        ...
        try:
            ...
            if self.is_private:
                # Deferred: flushed on return to NORMAL (set_mode) so the
                # on-disk index doesn't keep a dangling entry for a file

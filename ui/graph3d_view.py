...
void main() {
    ...
    float core = 1.0 - smoothstep(0.58, 0.68, hexDistance);
    float border = exp(-abs(hexDistance - 0.72) * 42.0);
    float innerRing = exp(-abs(radius - 0.43) * 34.0);
    float outerRing = exp(-abs(radius - 1.08) * 34.0);
    float segments = smoothstep(0.18, 0.78, 0.5 + 0.5 * sin(angle * 6.0));
    float halo = (1.0 - smoothstep(0.72, 1.55, radius)) * 0.30 * vGlow;

    float glyph = max(core * 0.62, border);
    glyph = max(glyph, innerRing * 0.72);
    glyph = max(glyph, outerRing * segments * (0.72 + 0.28 * vGlow));
    if (vState > 0.5) glyph *= 1.30;

    float alpha = clamp(max(glyph, halo), 0.0, 1.0);
    if (alpha < 0.012) discard;
    vec3 darkEnergy = vColor * (0.28 + core * 0.42);
    vec3 hotEnergy = mix(vColor, vec3(1.0), 0.68 + border * 0.22);
    vec3 finalColor = mix(darkEnergy, hotEnergy, clamp(glyph, 0.0, 1.0));

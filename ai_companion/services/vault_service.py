...
class VaultService(PrivacyMixin, BaseService):
    ...
    def _reconcile_index(self) -> None:
        ...
        A file removed during a private session is deleted from disk, but the
        index write is suppressed - so the entry survives into the next
        session and list_files() reports a file that is not there. Dropping
        stale entries on load keeps the index honest without ever recording
        what happened during the private session.
        """
        if self._vault_store is None:
            return
        stale = [
            key for key in list(self._vault_store.keys())
            if not (self._vault_root / key).exists()
        ]
        if not stale:
            return
        for key in stale:
            self._vault_store.delete(key)
        self._audit("index_reconciled", f"{len(stale)} missing file(s) dropped")
        self._persist_index()

    def _persist_index(self) -> None:
        """Write the vault index, unless the vault is private.

        VaultService was the last persistent service not using the shared
        privacy rule. Chat, memory, graph and code snapshots all refuse to
        write in private mode; the vault index did not, so removing a file
        during a private session still altered a file on disk.
        """
        if self._vault_store is None:
            return
    ...
    def remove_file(self, vault_path: str) -> bool:
        ...
        try:
            ...
            # Deleting the file is always honoured - refusing that would
            # trap data in the vault. But the index is only written in
            # normal mode, matching every other persistent store: private
            # mode means the session leaves no new trace on disk.
            self._persist_index()
